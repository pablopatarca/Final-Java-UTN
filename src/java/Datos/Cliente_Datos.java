...
public abstract class Cliente_Datos 
{
	public static Cliente ValidarCliente(String usuario, String pass) throws SQLException
	{
		...
		try
		{
                        //System.out.println("Ingresa en Validar Cliente");
	...
	public static boolean nuevoCliente(Cliente cli) throws SQLException
	{
		try{
			...
			PreparedStatement pstm = con.prepareStatement(
                                ...
                                + "(username, password, lname, name, address, phone, cell_phone, email)"
                                + " VALUES"
                                + "(?,?,?,?,?,?,?,?)");
			pstm.setString(1, cli.getUsuario());
			pstm.setString(2, cli.getPassword());
                        pstm.setString(3, cli.getApellido());
			pstm.setString(4, cli.getNombre());
			pstm.setString(5, cli.getAddress());
                        pstm.setString(6, cli.getTelefono());
                        pstm.setString(7, cli.getCelular());
                        pstm.setString(8, cli.getEmail());
                        */
			//pstm.executeUpdate();               
